...
public class MyPersistentTokenBasedRememberMeServices   extends AbstractRememberMeServices {
    ...
    private MyInMemoryTokenRepositoryImpl tokenRepository = new MyInMemoryTokenRepositoryImpl();
    ...
    protected UserDetails processAutoLoginCookie(String[] cookieTokens, HttpServletRequest request, HttpServletResponse response) {
        ...
        MyPersistentRememberMeToken token = tokenRepository.getTokenForSeries(presentedSeries);
        ...
        if (!presentedToken.equals(token.getTokenValue())) {
            ...
            tokenRepository.removeUserTokens(token.getUsername(),token.getRtaxi());
        ...
        MyPersistentRememberMeToken newToken = new MyPersistentRememberMeToken(token.getUsername(),
                token.getSeries(), generateTokenData(), new Date(),token.getRtaxi());

        try {
            tokenRepository.updateToken(newToken.getSeries(), newToken.getTokenValue(), newToken.getDate(),token.getRtaxi());
        ...
        UserDetails user = getUserDetailsService().loadUserByUsernameAndRtaxi(token.getUsername(),token.getRtaxi());
    ...
    protected void onLoginSuccess(HttpServletRequest request, HttpServletResponse response, Authentication successfulAuthentication) {
		...
		Long rtaxi=request.getAttribute("rtaxi")
        MyPersistentRememberMeToken persistentToken = new MyPersistentRememberMeToken(username, generateSeriesData(),
                generateTokenData(), new Date(),rtaxi);
        try {
    ...
    public void logout(HttpServletRequest request, HttpServletResponse response, Authentication authentication) {
        ...
        if (authentication != null) {
            tokenRepository.removeUserTokens(authentication.getName(),authentication.getProperties().get('rtaxi'));
        }
    }
    ...
    private void addCookie(MyPersistentRememberMeToken token, HttpServletRequest request, HttpServletResponse response) {
		
	  String[] test= new String[2];;
	  test[0] = token.getSeries().toString();
	  test[1] = token.getTokenValue().toString();
      setCookie(test, getTokenValiditySeconds(), request, response);
    }

    public void setTokenRepository(MyInMemoryTokenRepositoryImpl tokenRepository) {
