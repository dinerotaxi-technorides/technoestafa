...
import com.*
class HomeCompanyAccountEmployeeController {
	...
	def save={Pedir1Command command ->
		...
		if (command.hasErrors()) {
			...
			render  (view:"/homeCompanyAccountEmployee/pedir" ,model:[command:command])
			return
		}
		...
		if(isInCityEnabled){
			try{
				...
				if(pl.streetNumber!=null  ){
					...
					placeCompanyAccountEmployeeService.createFavoriteAndOperation(user,place1,place2,true,command.piso,command.departamento,command.comentarios,command.placeinput1,,command.placeinput2,devic.dev)
					...
					redirect(controller: "homeCompanyAccountEmployee", action: "pedir")
					return
				}
				...
				redirect(controller: "homeCompanyAccountEmployee", action: "pedir",model:[command:command])
				return
			}
			redirect(controller: "homeCompanyAccountEmployee", action: "complete")
		}else{
