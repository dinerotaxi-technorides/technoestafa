...
<script type="text/javascript">
	        $(document).ready(function() {
                         <jqgrid:grid
                                id="contact"
                                    url="'${createLink(action:'jq_online_company_account_user_list',controller:'radioTaxiCompanyAccount')}'"
                                        editurl="'${createLink(action:'jq_edit_company_account_user',controller:'radioTaxiCompanyAccount')}'"
                                colNames="'Empresa','Usuario','Contraseña','Nombre','Apellido','Telefono','Viajes','Habilitado','Cuenta Bloqueada','id'"
                                	 colModel="{name:'empresa',index:'patente', width:200, sortable:false,
                           			         editable:true, edittype:'select'  , editrules:{required:false},
                       			     editoptions:{dataUrl:'${createLink(action:'jq_get_company_account',controller:'radioTaxiCompanyAccount')}'}
                         ...
                         <jqgrid:grid
                         id="contactEmpresas"
                             url="'${createLink(action:'jq_online_company_account_list',controller:'radioTaxiCompanyAccount')}'"
                                 editurl="'${createLink(action:'jq_edit_company_account',controller:'radioTaxiCompanyAccount')}'"
                         colNames="'Email','Contraseña','Telefono','Empresa','CUIT','Habilitado','Cuenta Bloqueada','id'"
...
<body>
	...
	<div class="mainContent_">
		...
		<div class="con">
			...
			<g:render template="sidebar" model="['action':'search']" />
			<!-- DETAIL -->
