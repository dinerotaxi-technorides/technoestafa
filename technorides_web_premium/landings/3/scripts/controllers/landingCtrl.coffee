technoridesApp.controller 'landingCtrl' , ['$scope','$settings', '$api', '$apiHandler', '$apiAdapter', '$location', '$window', '$http', '$message', ($scope, $settings, $api, $apiHandler, $apiAdapter, $location, $window, $http, $message, $cookieStore) ->
  ...
  $scope.sendTrial = (mail) ->
    mail.plan = "ADWORDS LANDING 3"
    $api.send 'freeTrial', mail, ->
      location.href = "/thankyou.html##{$scope.lang}"
      # $location.path "/thankyou"
      # $http.get "https://www.googleadservices.com/pagead/conversion/959494099/?label=8BInCKPepVcQ0-_CyQM&amp;guid=ON&amp;script=0"
